...
parser.add_argument('config', help='Path to JSON configuration file with cluster parameters')
...
# Authenticate with Gaia if credentials are available
if GAIA_USER_NAME and GAIA_USER_PASSWORD:
    print("Authenticating with Gaia archive...")
    Gaia.login(user=GAIA_USER_NAME, password=GAIA_USER_PASSWORD)
...
if os.path.exists(data_file):
    ...
    # We filter by proper motion to isolate cluster members from background/foreground
    query = f"""
...
plt.savefig('hr_diagram_47_tuc.png', dpi=600, bbox_inches='tight', facecolor='black', edgecolor='none')